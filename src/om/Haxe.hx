...
/**
    Compile time tools accessible from non-macro context.
*/
class Haxe {
	...
	macro public static function definedValue( key : String, ?def : String ) : ExprOf<String> {
		var v = Context.defined( key ) ? Context.definedValue( key ) : def;
		return macro $v{v};
	}
	...
	*/
	macro public static function version() : ExprOf<String> {
		var v = haxe.macro.Context.definedValue( 'haxe_ver' );
		return macro $v{v};
	}
